...
function paraDatetimeLocal(valor?:string){
  ...
  if(Number.isNaN(dataHora.getTime()))return valor.slice(0,16)
  const pad=(valor:number)=>String(valor).padStart(2,'0')
  return `${dataHora.getFullYear()}-${pad(dataHora.getMonth()+1)}-${pad(dataHora.getDate())}T${pad(dataHora.getHours())}:${pad(dataHora.getMinutes())}`
}
