...
.overlay.confirmacao{z-index:30}.modal-confirmacao{max-width:400px;padding:1.25rem;display:flex;flex-direction:column;align-items:center;gap:1rem;background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:calc(var(--radius) + 6px)}.modal-confirmacao h2{margin:0;color:var(--text)!important;font-size:1.1rem;font-weight:700}.modal-confirmacao p{margin:0;color:var(--text-dim)!important;line-height:1.45}.modal-confirmacao .modal-rodape{width:100%;margin-top:.25rem}.modal-confirmacao .modal-rodape>*{flex:1}/* Cores explícitas: o style.css global usa --text-h, que não acompanha o tema operacional. */
.tela{background:var(--bg)!important;color:var(--text)}
.topbar{background:var(--surface)!important}
.topbar h1,.modal h2,.modal-confirmacao h2,.card strong{color:var(--text)!important}
.card{background:var(--surface);color:var(--text)}
.periodo{color:var(--text-dim)}
.modal{background:var(--surface);color:var(--text)}
.modal-confirmacao{background:var(--surface)!important;color:var(--text)!important}
.modal-confirmacao p,.descricao{color:var(--text-dim)!important}
.voltar,.fechar,.secundario,.icone{background:var(--input-bg);color:var(--text)}</style>