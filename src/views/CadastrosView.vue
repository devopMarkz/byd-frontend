...
function selecionarImagem(evento: Event) { const arquivo = (evento.target as HTMLInputElement).files?.[0]; if (!arquivo) return; if (arquivo.size > 1024 * 1024) { erro.value = true; mensagem.value = 'A imagem deve ter no máximo 1 MB.'; return }; const leitor = new FileReader(); leitor.onload = () => imagemBase64.value = String(leitor.result); leitor.readAsDataURL(arquivo) }
